...
namespace common
{
	...
	int Redis::redis_lrem(const char* key, const char* value, const int length)
	{
		// LOG_DEBUG("redis key : " << key);
		// LOG_DEBUG("redis value : " << common::printByHex(value, length));

		redisReply* r = (redisReply*)redisCommand(c, "LREM %s %d %b", key, 0, value, (size_t)length);

		if(r == NULL || r->type == REDIS_REPLY_ERROR || (r->type == REDIS_REPLY_STATUS && r->str != REDIS_OK))
		{
			LOG_DEBUG("redis LREM error");
			freeReplyObject(r);
			return -1;
		}
		...
		{
			i = r->integer;
			LOG_DEBUG("redis LREM return ok, number of delete value is " << r->integer);
			freeReplyObject(r);
		}
	...
	int Redis::redis_rpoplpush(const char* key_src, const char* key_dest, char* value, int* len)
	{
		...
		{
			LOG_DEBUG("redis RPOPLPUSH return ok, value = " << common::printByHex(r->str, r->len));
	...
	int Redis::redis_select(const int db)
	{
		...
		if(r == NULL || (r->type == REDIS_REPLY_STRING && strcasecmp(r->str, "OK") != 0))
		{
