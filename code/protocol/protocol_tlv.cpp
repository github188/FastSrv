...
namespace protocol
{
	...
	{
		LOG_DEBUG("parseHead :" << sizeof(tlv::Head));
		common::printByHex(data, sizeof(tlv::Head));
	...
	{
		LOG_DEBUG("parseBody :" << ptrHead->len);
		common::printByHex(data + sizeof(tlv::Head), ptrHead->len);
	...
	void PacketIn::printDetail()
	{
		...
		LOG_DEBUG("Head : " << sizeof(tlv::Head));
		common::printByHex((char*)ptrHead, sizeof(tlv::Head));

		LOG_DEBUG("TLV group size : " << vecTLV.size());
		for(unsigned int i = 0; i < vecTLV.size(); i++)
		{
			LOG_DEBUG("tlv type :");
			common::printByHex(&vecTLV[i].type, 1);

			LOG_DEBUG("tlv len :");
			common::printByHex(&vecTLV[i].len, 1);

			LOG_DEBUG("tlv value :" << (int)vecTLV[i].len);
			common::printByHex(vecTLV[i].value, (int)vecTLV[i].len);
		}

		LOG_DEBUG("CRC : ");
		common::printByHex((char*)crc, sizeof(unsigned short));
	}
	...
	{
		return string((char*)ptrHead->dev_id);
	}
	...
	void PacketOut::build()
	{
		...
		ptrHead->len = len - sizeof(tlv::Head);
	...
	void PacketOut::printDetail()
	{
		...
		LOG_DEBUG("CRC : ");
		common::printByHex((char*)&crc, sizeof(crc));
	}
