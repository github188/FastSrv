...
namespace PS_FW
{
	...
	public:
		void init()
		{
			...
			printf("\n[PS_FW::ChannelImpWin]: init() success ! \n");
		}
		...
		void close(int& sockfd)
		{
			...
			printf("\n[PS_FW::ChannelImpWin]: close() success ! \n");
		}
		...
		int acceptConnInit(const char* ip,const int port,int& sockId)
		{
			...
			printf("\n[PS_FW::ChannelImpWin]: self socket is %d ! \n",s);
			...
			printf("\n[PS_FW::ChannelImpWin]: acceptConnInit() success , listen on %s:%d now ! \n",ip,port);
		...
		int acceptConn(const int sockId,int& sockPeerId)
		{
			...
			printf("\n[PS_FW::ChannelImpWin]: peer socket is %d ! \n",s);
			...
			if(sockPeerId < 0)
			{
				printf("\n[PS_FW::ChannelImpWin]: acceptConn failed with error %d \n",WSAGetLastError());
				return -1;
			}
		...
		int recvData(const int sockId,char* szBuf,const int len,int& realLen)
		{
			...
			printf("\n[PS_FW::ChannelImpWin]: recvData return szBuf = %s return 0 ! \n",szBuf);
			return 0;
		}
